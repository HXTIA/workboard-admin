    ...
    <div class="login-wrapper-form">
      ...
      >
        ...
        <el-form-item
          prop="captcha"
          label="验证码"
          class="login-wrapper-form-captcha"
        >
        ...
        <el-button
          type="primary"
          @click="login"
          class="login-wrapper-form-loginBtn"
          >登录</el-button
        >
...
const login = async () => {
  loginForm.value.validate(async (valid) => {
    if (valid) {
      ...
      const { ok, isComplete } = await loginReq({
      ...
      if (isComplete) {
        ...
        router.push({ path: '/userInfo', replace: true });
      }
    } else {
...
loginData.username = JSON.parse(
  JSON.stringify(props.registerSuccessData.username)
);
loginData.password = JSON.parse(
  JSON.stringify(props.registerSuccessData.password)
);
